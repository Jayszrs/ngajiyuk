...
<div class="modal" id="import-students" aria-hidden="true"><div class="modal-card"><div class="modal-head"><div><p class="eyebrow">IMPOR DATA</p><h2>Impor Siswa dari Excel</h2></div><button class="modal-close" type="button" data-modal-close>&times;</button></div><form action="<?= url('api/import/students.php') ?>" method="post" enctype="multipart/form-data" data-ajax class="form-grid"><?= csrf_field() ?><div class="field full"><label>File XLSX atau CSV *</label><input class="input" type="file" name="file" accept=".xlsx,.csv" required><small class="muted">Kolom Nama dan NIS akan dideteksi otomatis.</small></div><div class="field"><label>Kelas Default</label><select class="select" name="kelas"><?php foreach(all_class_names() as $class): ?><option value="<?= e($class) ?>"><?= e($class) ?></option><?php endforeach; ?></select></div><div class="field"><label>Level Default</label><select class="select" name="level"><?php for($level=1;$level<=9;$level++): ?><option value="<?= $level ?>"><?= e(level_name($level)) ?></option><?php endfor; ?></select></div><div class="field full"><button class="btn btn-primary" type="submit">Mulai Impor Siswa</button></div></form></div></div>
